...
export class FlashcardDndService {
  ...
  constructor() {
    ...
    if (state.currentSetId) {
      ...
      // Also update the SetSelectionService to keep them in sync
      const selectedSet = state.flashcardSets.find(
        (s) => s.id === state.currentSetId,
      )
      if (selectedSet) {
        this.setSelectionService.setSelectedSet(selectedSet, true) // Skip notification to prevent loops
        console.log(
          `Initialized SetSelectionService with: ${selectedSet.title}`,
        )
      }
      ...
      this.setSelectionService.setSelectedSet(state.flashcardSets[0], true) // Skip notification to prevent loops
      console.log(
        `No selected set ID found, defaulting to first set: ${state.flashcardSets[0].id}`,
      )
      console.log(
        `SetSelectionService initialized with: ${state.flashcardSets[0].title}`,
      )
    }
  }
  ...
  async loadFlashcardSets(): Promise<void> {
    try {
      ...
      if (sets.length > 0) {
        ...
        this.setSelectionService.setSelectedSet(sets[0], true) // Skip notification to prevent loops
        console.log(`Auto-selected first set: ${sets[0].title} (${sets[0].id})`)
        console.log(`SetSelectionService should now have: ${sets[0].title}`)
      }
    } catch (error) {
  ...
  async loadSet(setId: string | null): Promise<void> {
    ...
    try {
      ...
      if (set) {
        ...
        this.setSelectionService.setSelectedSet(set, true) // Skip notification to prevent loops
      }
    } catch (error) {
  ...
  createNewSet(
    ...
    const newSet: CreateFlashcardSetDto = {
      iconId: { value: icon_id } as Record<string, any>,
      title: title,
    }
  ...
  setNewFlashcards(cards: { front: string; back: string }[]): void {
  ...
  selectSet(setId: string): void {
    ...
    // Find the set and update SetSelectionService to keep them in sync
    const set = this.flashcardSets().find((s) => s.id === setId)
    if (set) {
      this.setSelectionService.setSelectedSet(set, true) // Skip notification to prevent loops
    }
  ...
  forceUpdateSelectedSetCards(cards: Flashcard[]): void {
    ...
    this.flashcardSets.update((sets) => {
      ...
      // Also update the SetSelectionService with the updated set
      if (updatedSet) {
        this.setSelectionService.setSelectedSet(updatedSet, true) // Skip notification to prevent loops
      }
  ...
  clearSelectedSet(): void {
    ...
    this.setSelectionService.setSelectedSet(null, true) // Skip notification to prevent loops
...
export { FlashcardDndService as FlashcardCDKService }
